...
const insertIntoCollection = (error, db, collectionName, data) => {
  db.collection(collectionName).insertOne(data, (error, result) => {
    if (error) {
      return error;//console.log('Unable to insert todo:', error);
    }

    return JSON.stringify(result.ops, undefined, 2);
  });
}
...
MongoClient.connect('mongodb://localhost:27017/TodoApp', (error, db) => {
  ...
  if (error) {
    return error;//console.log('Unable to connect to mongodb server');
  }
